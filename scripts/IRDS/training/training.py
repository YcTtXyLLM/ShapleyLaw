...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        # Optional In-Run Data Shapley attribution hook.
        #
        # The concrete implementation lives in the entrypoint script
        # pretrain_gpt_inrun_shapley.py. Since that script is executed as
        # __main__, importing it here can cause a second module load or circular
        # imports. We therefore look up maybe_run_inrun_shapley from __main__.
        #
        # This hook runs after the normal training step and optimizer update,
        # using the latest training micro-batch cached by forward_step(). It
        # performs its own attribution forward/backward and then clears grads,
        # so it should not change the completed optimizer step.
        if (
            not skipped_iter
            and os.environ.get("MEGATRON_INRUN_SHAPLEY_ENABLE", "0") == "1"
        ):
            main_module = sys.modules.get("__main__", None)
            maybe_run_inrun_shapley = getattr(
                main_module, "maybe_run_inrun_shapley", None
            ) if main_module is not None else None

            if maybe_run_inrun_shapley is not None:
                shapley_valid_data_iterator = (
                    valid_data_iterator[0]
                    if isinstance(valid_data_iterator, list)
                    else valid_data_iterator
                )
                try:
                    maybe_run_inrun_shapley(
                        iteration,
                        model,
                        optimizer,
                        shapley_valid_data_iterator,
                    ...
                    print_rank_0(
                        f"[InRunShapley] hook failed at iteration={iteration}: "
...
def evaluate_and_print_results(prefix, forward_step_func,
    ...
    GLOBAL_CSV_FILE = os.environ.get("EVAL_OUTPUT_FILE") or os.path.join(
        GLOBAL_CSV_DIR, "validation_lm_loss_all.csv"
    )

    if is_last_rank():
        os.makedirs(os.path.dirname(GLOBAL_CSV_FILE) or GLOBAL_CSV_DIR, exist_ok=True)
        file_exists = os.path.exists(GLOBAL_CSV_FILE)
    ...
    for key in total_loss_dict:
        ...
        # ===================== NEW: write to CSV =====================
        if is_last_rank():
            with open(GLOBAL_CSV_FILE, "a", newline="") as f:
                writer_csv = csv.writer(f)
                if not file_exists:
                    writer_csv.writerow(
                        ["task_name", "iteration", "prefix", "key", "lm_loss", "ppl"]
                    )
                    file_exists = True
                writer_csv.writerow(
                    [task_name, iteration, prefix, key, loss_val, ppl]
                )
        # =============================================================
