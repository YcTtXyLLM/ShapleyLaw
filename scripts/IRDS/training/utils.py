...
def get_batch_on_this_tp_rank(data_iterator):
    ...
    def _metadata_or_sentinel(data, key, shape):
       """Return CUDA metadata tensor, or -1 sentinel if key is unavailable."""
       if data is not None and key in data and data[key] is not None:
           value = data[key]
           if not torch.is_tensor(value):
               value = torch.as_tensor(value, dtype=torch.int64)
           value = value.to(device=torch.cuda.current_device(), dtype=torch.int64, non_blocking=True)
           # Most dataloaders collate language_id as [micro_batch_size]. If a
           # scalar slips through, expand it to one value per sequence.
           if value.dim() == 0:
               value = value.view(1).expand(shape).contiguous()
           return value
       return torch.full(shape, -1, dtype=torch.int64, device=torch.cuda.current_device())

    def _maybe_attach_metadata(batch, language_id, dataset_id):
       """Attach metadata only when all entries are valid non-negative ids."""
       if not inrun_shapley_enabled:
           return
       if language_id is not None and bool((language_id >= 0).all().item()):
           batch['language_id'] = language_id.long()
       if dataset_id is not None and bool((dataset_id >= 0).all().item()):
           batch['dataset_id'] = dataset_id.long()

    metadata_shape = (args.micro_batch_size,)
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
       ...
       language_id = None
       dataset_id = None
       if inrun_shapley_enabled:
           language_id = _metadata_or_sentinel(data, "language_id", metadata_shape)
           dataset_id = _metadata_or_sentinel(data, "dataset_id", metadata_shape)
       ...
       _maybe_attach_metadata(batch, language_id, dataset_id)
       ...
       dataset_id = None
       if inrun_shapley_enabled:
           language_id = torch.empty(metadata_shape, dtype=torch.int64, device=torch.cuda.current_device())
           dataset_id = torch.empty(metadata_shape, dtype=torch.int64, device=torch.cuda.current_device())
